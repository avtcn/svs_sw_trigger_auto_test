...
private:
    // Joe: save a copy of latest frame from SVS camera 
    CRITICAL_SECTION cscopybufferinfo;
    SV_BUFFER_INFO copyBufferInfo = { 0 };
    bool bCopyBufferReady = false;

public:
    // Joe: interfaces to local buffer copy
    bool EnableSoftwareTriggerMode();
    bool EmitSoftwareTriggerSignal();
    bool CheckSoftwareTriggerResult();
    bool GetSoftwareTriggerResult(SV_BUFFER_INFO* bufferInfo);
